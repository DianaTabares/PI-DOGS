...
const postDog = async (req, res) => {
    try {
        const { name, image, height, weight, yearsOfLife, temperaments } = req.body // recibimos por body los datos que nos envian desde el front
        ...
        if (!name || !height || !weight || !yearsOfLife) {
            return res.status(404).json({ error: "No se han ingresado todos los datos" })
        }
